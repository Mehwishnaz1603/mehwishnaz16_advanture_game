...
class Player { name: string; fuel: number = 100;

    constructor(name: string) {this.name = name;}
    decreaseFuel() { this.fuel -= 20;
        if ( this.fuel < 0) { this.fuel = 0 }
    }
     increaseFuel() { this.fuel = 100 }
}
class Opponent { name: string; fuel: number = 100;
     constructor(name: string) { this.name = name }
    decreaseFuel() { this.fuel -= 20;
        if (this.fuel < 0) { this.fuel = 0;}
    }
...
async function chooseOpponent(): Promise<string> {
...
async function getPlayerAction(): Promise<string> {
